...
tissue=$(echo $2 | cut -f2 -d"=")
res=$(echo $3 | cut -f2 -d"=")
name=$(echo $4 | cut -f2 -d"=")
out=$(echo $5 | cut -f2 -d"=")
pval=$(echo $6 | cut -f2 -d"=")
dir=$(echo $7 | cut -f2 -d"=")
in=$(echo $8 | cut -f2 -d"=")
prtT=0
...
prtT=1
	...
	echo "You have selected CIS interaction analysis ..."	
		...
		awk -F"[_/]" '{print $2"\t"$3"\t"$4"\t"$7"\t"$8"\t"$9}' $dir/$cell/cis.$res\_iced.sorted.txt | awk -F" " '{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$5-$2"\t"$7}'  > $out/$cell.cis_tmp/tmp
		cd $out/$cell.cis_tmp
	...
	echo "You have selected genome-wide TRANS pairwise analysis ..."
		...
		awk -F"[_/]" '{print $2"\t"$3"\t"$4"\t"$7"\t"$8"\t"$9}' $dir/$cell/trans.$res\_iced.sorted.txt | awk -F" " '{print $1$4".txt""\t"$1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$5-$2"\t"$7}' > $out/tmp ; mv $out/tmp $out/$cell.trans.tmp/
	...
	echo "-------------------------------------------"	
		echo -e "\n## preparing data ..."
		awk -F"[_/]" '{print $2"\t"$3"\t"$4"\t"$7"\t"$8"\t"$9}' $dir/$cell/trans.$res\_iced.sorted.txt | awk -F" " '{print $1$4".txt""\t"$1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$5-$2"\t"$7}' > $out/tmp
		cd $out
