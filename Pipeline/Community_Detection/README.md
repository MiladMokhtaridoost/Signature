...
This script is intended solely for visualization purposes, as explained in the following paragraph. It constructs a chromosomal structure by connecting consecutive mapped bins. The input for this script is generated by the code in step 2 (**merge_networks.R**). The resulting output is a two-column table that can be used as input to Gephi for visualization. Below is an example of the output format for one dataset:
